...
async function run() {
  try {
    ...
    app.put("/api/v1/donations/:id", async (req, res) => {
      ...
      console.log(id, updateDonation);
      const setDonation = {
        $set: {
          image: updateDonation.image,
          title: updateDonation.title,
          amount: updateDonation.amount,
          description: updateDonation.description,
          category: updateDonation.category,
        },
      };
      console.log("SET DONATION", setDonation);
